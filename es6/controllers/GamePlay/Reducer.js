        ...
        switch (action.type) {
            case TYPES.RESTART:
                newState = newState.withMutations((m) => {
                    let player1 = m.get('player1'), player2 = m.get('player2');
                    player1.setScore(0);
                    player2.setScore(0);
                    player1.state = null;
                    player2.state = null;
            ...
            case SPLASH_TYPES.START:
                newState = newState.withMutations((m) => {
                    let isSimulated = action.data === GAME_MODES.COMPUTER_VS_COMPUTER, player1 = new ComputerPlayer(isSimulated ? "Computer 1" : "Computer"), player2 = isSimulated ? new ComputerPlayer("Computer 2") : new UserPlayer("You");
                    m.set('visible', true)
                        .set('round', 1)
            ...
            case TYPES.CHOICE:
                newState = newState.withMutations((m) => {
                    let player1 = m.get('player1'), player2 = m.get('player2'), round = m.get('round'), maxRounds = m.get('maxRounds'), gameWinner = null, winner, play = player1.play();
            ...
            case TYPES.NEXT_ROUND:
                newState = newState.withMutations((m) => {
                    let round = m.get('round'), maxRounds = m.get('maxRounds');
